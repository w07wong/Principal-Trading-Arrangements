...
from env.agent_env import AgentEnv_CVAR
...
def run(args):
    ...
    for _ in range(args.n_games):
        ...
        agent_env = make_vec_env(lambda: AgentEnv_CVAR(principal_model, cvar_alpha=args.cvar_alpha, num_cvar_runs=args.num_cvar_runs, 
                                                        theta=args.theta, lam=args.lam, sigma=args.sigma, T=args.T), n_envs=96)

        # Define agent model to train.
        agent_model = PPO2(CustomPolicy, agent_env, verbose=1, lam=0.8, gamma=0.99, learning_rate=args.agent_lr, n_steps=96)

        # # Train agent model.
        agent_model.learn(total_timesteps=args.agent_timesteps)

        # Create gym environments, passing in the pretrained agent.
        # principal_env =  make_vec_env(lambda: PrincipalEnv(agent_model, agent_env, theta=theta, lam=lam, sigma=sigma, T=T), n_envs=1)
        ...
        # principal_model.learn(total_timesteps=principal_timesteps)

    if not os.path.exists(args.save_dir):
        os.makedirs(args.save_dir)
    if not args.disable_agent:
        agent_model.save(f'{args.save_dir}/agent_cvar')
    if not args.disable_principal:
        principal_model.save(f'{args.save_dir}/principal_cvar')
...
if __name__=="__main__":
    ...
    parser.add_argument('--num_cvar_runs', type=int, default=20)
